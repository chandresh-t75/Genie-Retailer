...
const ProductOrderCard = ({ product }) => {
  ...
  return (
    <View className="max-w-[340px] flex-row relative items-center justify-between bg-white gap-[15px]  rounded-3xl shadow-2xl  px-[20px] h-max py-[20px]">
      {prod?.latestMessage?.sender?.type === "UserRequest" &&
        prod?.unreadCount > 0 && (
          <View
            className="w-[20px] h-[20px] flex justify-center items-center  rounded-full absolute -top-[10px] right-[20px]"
      ...
      <View className="w-[95px] h-[95px]  rounded-[15px]">
        {prod?.requestId?.requestImages?.length > 0 ? (
          <Image
            source={{ uri: prod.requestId.requestImages[0] }}
      ...
      <View className="w-10/12 px-[10px]">
        <View className="flex flex-wrap w-10/12 pb-1 ">
          <Text
            className="text-[14px] w-full text-[#2E2C43]"
            style={{ fontFamily: "Poppins-Regular" }}
          >
            {prod?.requestId?.requestDescription}
          </Text>
        </View>
        ...
        <View className="flex-row py-1 w-10/12 ">
          ...
          <Text
            className="text-[12px] text-[#70B241] "
            style={{ fontFamily: "Poppins-SemiBold" }}
          >
            {prod?.requestId?.expectedPrice>0
              ? `Rs. ${prod?.requestId?.expectedPrice}`
              : "Na"}
          </Text>
        </View>
        <View className="flex-row gap-[8px] w-10/12">
          <View className="flex-row items-center  gap-[8px]">
            <Time size={13} />
            <Text
              className="text-[12px] text-[#001B33]"
              style={{ fontFamily: "Poppins-Regular" }}
            >
              {formattedTime}
            </Text>
